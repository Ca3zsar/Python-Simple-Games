...
def drawBoard(board):
    ...
    tensDigitsLine = '  '  # Initial space for the numbers down the left site of the board
    for i in range(1, 6):
...
def getRandomChests(numChests):
    ...
    while len(chests) < numChests:
        ...
        if newChest not in chests:  # Make sure a chests is not already here.
...
def makeMove(board, chests, x, y):
    # Change the board data structure with a sonar device character. Remove treasure chets from the chests list as they are found
    ...
    for cx, cy in chests:
        ...
        if distance < smallestDistance:  # We want the closest treasure chests
            smallestDistance = distance

    smallestDistance = round(smallestDistance)

    if smallestDistance == 0:
        # xy is directly on a tresure chest!
        chests.remove([x, y])
